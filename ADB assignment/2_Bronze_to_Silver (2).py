...
def explode_(dataframe: DataFrame, explode_column: str, alias: str, column: List = []) -> DataFrame:
    return dataframe.select(*column, explode(col(explode_column)).alias(alias))
movieGenreDF = explode_(extractedDF, "genres", "genres", ["Id"])
